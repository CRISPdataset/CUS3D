...
from .custom200 import Custom200Dataset


class S3DISDataset(Custom200Dataset):
    ...
    def load(self, filename):
        xyz, rgb, semantic_label, _, _, _ = torch.load(filename)
        # subsample data
    ...
    def transform_test(self, xyz, rgb, semantic_label):
        if not self.x4_split:
            return super().transform_test(xyz, rgb, semantic_label)
    ...
    def collate_fn(self, batch):
        ...
        # assume 1 scan only
        (scan_id, coord, coord_float, feat, semantic_label, clip_fcs) = batch[0]
        scan_ids = [scan_id]
        coords = coord.long()
