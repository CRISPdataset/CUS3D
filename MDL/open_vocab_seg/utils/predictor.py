...
class OVSegPredictor(DefaultPredictor):
    ...
    def __call__(self, original_image, class_names):
        ...
        with torch.no_grad():  # https://github.com/sphinx-doc/sphinx/issues/4258
            ...
            return predictions
    def get_pixel_label(self, image):
            pred = self(image, ['a', 'b'])
            image_features = pred['fc']
    ...
    def get_text_features(self, class_names):
        with torch.no_grad():
            return self.model.get_text_features(class_names)
...
class VisualizationDemo(object):
    ...
    def get_text_feature(self, class_names):
        return self.predictor.get_text_features(class_names)
    def get_mask_feature(self, image, class_names):
        predictions = self.predictor(image, class_names)
        return predictions["mask_fc"]        
    def vis(self, image, class_names, pred_mask, thr=10):
        image = image[:, :, ::-1]
    ...
    def run_on_image(self, image, class_names):
        ...
        if "sem_seg" in predictions:
            ...
            image_features = predictions["fc"]
            odp_fc = predictions["odp_fc"]
            text_features = predictions["text_fc"]
            blank_area = (r[0] == 0)
        ...
        return predictions, vis_output, image_features, text_features, odp_fc
...
class SAMVisualizationDemo(object):
    ...
    def run_on_image(self, image, class_names):
        ...
        #     image_features = self.clip_model.encode_image(imgs.cuda())
        ...
        # select_cls = torch.zeros_like(class_preds).cpu()
        ...
        fc = self.clip_model.encode_image(imgs.cuda())
        print(fc.shape)
        exit(1)
        return None, None, fc, None