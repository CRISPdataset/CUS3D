...
class CustomDataset(Dataset):
    ...
    def transform_test(self, xyz, rgb, semantic_label, instance_label):
        ...
        xyz = xyz_middle * self.voxel_cfg.scale
        xyz -= xyz.min(0)
        valid_idxs = np.ones(instance_label.shape[0], dtype=bool)
        instance_label = self.getCroppedInstLabel(instance_label, valid_idxs)
        return xyz, xyz_middle, rgb, semantic_label, instance_label
    
    def mask_unseen_points(self, xyz, rgb, sem, ins, clip_fc):
        idx_list = []
        ...
        for idx in idx_list:
            ...
            ins = ins[idx]
            clip_fc = clip_fc[idx]
        return (xyz, rgb, sem, ins), clip_fc
    
    def reserve_unseen_points(self, xyz, rgb, sem, ins, clip_fc):
        idx_list = []
        for label in self.mask_labels:
            # sem = sem[sem != self.mask_labels[i]]
            idx = np.nonzero((sem == label))[0]
    ...
    def __getitem__(self, index):
        ...
        ins_data = np.load(f"/home/wxj/code/P2P/ov-seg/data/scannet/scannet_data/{scan_id}_ins_label.npy").astype(np.int32)
        ...
        data = (data[0], data[1], sem_data, ins_data)
        
        
        # mask
        # mask_ids = self.get_mask_ids(sem_data)
        ...
        return (scan_id, coord, coord_float, feat, semantic_label, instance_label, inst_num,
                inst_pointnum, inst_cls, pt_offset_label, clip_fc)
    ...
    def collate_fn(self, batch):
        ...
        for data in batch:
            ...
            (scan_id, coord, coord_float, feat, semantic_label, instance_label, inst_num,
             inst_pointnum, inst_cls, pt_offset_label, clip_fc) = data
