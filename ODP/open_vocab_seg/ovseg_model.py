...
class OVSegDEMO(MaskFormer):
    ...
    def forward(self, batched_inputs):
        ...
        mask_cls_results = outputs["pred_logits"] # torch.Size([100, 6])
        mask_pred_results = outputs["pred_masks"] # torch.Size([100, 160, 216])
        ...
        # print(mask_pred_results[0].shape): torch.Size([100, 640, 864])
        processed_results = []
        for mask_cls_result, mask_pred_result, input_per_image, image_size in zip(
            mask_cls_results, mask_pred_results, batched_inputs, images.image_sizes
        ):
            ...
            r, regions, fc, text_fc, odp_fc = self.demo_inference(mask_cls_result, mask_pred_result, image, class_names)
            height = input_per_image.get("height", image_size[0])
            width = input_per_image.get("width", image_size[1])
            r = sem_seg_postprocess(r, image_size, height, width)
            fc = sem_seg_postprocess(fc, image_size, height, width)
            odp_fc = sem_seg_postprocess(odp_fc, image_size, height, width)
            processed_results.append({"sem_seg": r, "fc": fc, "text_fc": text_fc, "odp_fc": odp_fc})
            
        return processed_results

    def get_text_features(self, class_names):
        return self.clip_adapter.get_text_features(class_names)



    def demo_inference(self, mask_cls, mask_pred, image, class_names):
        mask_cls = F.softmax(mask_cls, dim=-1)[..., :-1]
        mask_pred = mask_pred.sigmoid() #[99, H, W]
        
        # print(mask_pred[:,0,0])
        # mask_pred = F.softmax(mask_pred*1000, dim=0)
        # print(mask_pred[:,0,0])
        regions = None
        if self.clip_ensemble:
            clip_cls, regions, valid_flag, features, text_features = self.clip_adapter(
                image, class_names, mask_pred, normalize=True
            )
            if clip_cls is None:
                clip_cls = torch.empty(0, mask_cls.shape[-1] + 1, device=self.device)
        ...
        # print(select_cls.shape) # torch.Size([99, 5])
        # print(bin_mask.shape) # ([99, 640, 853])
        # print(features.shape) # torch.Size([99, 768])
                
        semseg = torch.einsum("qc,qhw->chw", select_cls, bin_mask.float())
        semseg_fc = torch.einsum("qc,qhw->chw", features, bin_mask.float())
        
        
        mask_label = mask_cls.argmax(dim=1)
        global_fcs = features.mean(dim=0)
        
        
        odp_fcs = torch.full_like(semseg_fc, 0)
        ...
        for i in range(mask_pred.shape[1]):
            for j in range(mask_pred.shape[2]):
                probs = mask_pred[:, i,j]
                # print(probs.shape)
                probs_ids = (probs>self.clip_adapter.mask_thr).nonzero().squeeze()
                # print(probs_ids)
                prob_labels = mask_label[probs_ids].reshape(-1)
                counts = torch.bincount(prob_labels, weights=mask_pred[probs_ids,i,j].reshape(-1))
                if counts.numel() == 0:
                    fcs = global_fcs
                else:
                    max_label = torch.argmax(counts)
                    #筛选等于max-label的mask
                    idx = (prob_labels == max_label).nonzero().squeeze()
                    # 这些id就是要计算feature的id
                    fcs = features[probs_ids].sum(dim=0)
                odp_fcs[:, i,j] = fcs
                #probs
        # # mask_pred = torch.where(mask_pred > self.clip_adapter.mask_thr, mask_pred, torch.full_like(mask_pred, 0))
        # # 过滤大于阈值的mask_pred
        
        # # bin_mask_st = torch.softmax(1000*mask_pred.permute(1,2,0), dim=-1).permute(2, 0, 1) 
        # # Object-level denoise
        
        # # 分pixel进行过滤
        # valid_indexes = (mask_pred>self.clip_adapter.mask_thr).nonzero()
        # for i in range(valid_indexes.shape[0]):
        #      valid_indexes[i]
        # counts = torch.bincount(mask_label)
        # max_label = torch.argmax(counts)
        
        # # 筛选掉不是此label的mask
        # idx = (mask_label == max_label).nonzero().squeeze()
        # features = torch.einsum("qc,qhw->chw", features[idx, :], bin_mask[idx, :].float())
        # print(semseg.shape) # torch.Size([5, 640, 853])
        return semseg, regions, semseg_fc, text_features, odp_fcs
