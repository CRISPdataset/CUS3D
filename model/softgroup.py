...
class SoftGroup(nn.Module):
    ...
    def __init__(self,
        ...
        self.to_high_mlp1 = MLP(channels, 64, norm_fn=norm_fn, num_layers=2)
        self.to_high_mlp2 = MLP(64, 256, norm_fn=norm_fn, num_layers=2)
        self.to_high_mlp3 = MLP(256, 768, norm_fn=norm_fn, num_layers=2)
        self.to_high_mlp = nn.Sequential(self.to_high_mlp1, self.to_high_mlp2, self.to_high_mlp3)
        
        self.to_low_mlp1 = MLP(768, 256, norm_fn=norm_fn, num_layers=2)
        self.to_low_mlp2 = MLP(256, 64, norm_fn=norm_fn, num_layers=2)
        self.to_low_mlp3 = MLP(64, channels, norm_fn=norm_fn, num_layers=2)
        self.to_low_mlp = nn.Sequential(self.to_low_mlp1, self.to_low_mlp2, self.to_low_mlp3)
    ...
    def forward(self, batch, return_loss=False, text_features=None):
        if return_loss:
            return self.forward_train(**batch, text_features=text_features)
        else:
            return self.forward_test(**batch)
    
    def get_cos_labels(self, image_features, text_features):
        a = image_features / image_features.norm(dim=-1, keepdim=True)
        b = text_features / text_features.norm(dim=-1, keepdim=True) # 99, 768
        similarity = (100.0 * a @ b.T).softmax(dim=-1)
    ...
    def forward_train(self, batch_idxs, voxel_coords, p2v_map, v2p_map, coords_float, feats,
                      ...
                      pt_offset_labels, spatial_shape, batch_size, clip_fcs, **kwargs):
        losses = {}
        if self.with_coords:
        ...
        #TODO: 验证
        hi_output_feats = self.to_high_mlp(output_feats)
        ...
        cos_labels = self.get_cos_labels(hi_output_feats, text_features)
        text_loss = self.text_loss(cos_labels, semantic_labels)
        losses.update(text_loss)
        
        output_feats = self.to_low_mlp(hi_output_feats)
        ...
        if not self.semantic_only:
            ...
            instance_batch_idxs, cls_scores, iou_scores, mask_scores, _ = self.forward_instance(
                inst_feats, inst_map)
        ...
        # TODO: feature loss
        # feature_loss = self.feature_loss(clip_features=clip_fcs, model_features=output_feats)
        # losses.update(feature_loss)
        return self.parse_losses(losses)
    
    def feature_loss(self, clip_features, model_features):
        losses = {}
        # losses['feature_loss'] = 
        clip_n2 = torch.norm(clip_features, p=2, dim=1, keepdim=True)
        model_n2 = torch.norm(model_features, p=2, dim=1, keepdim=True)
        D = -(clip_features / clip_n2) * (model_features / model_n2)
        loss = torch.mean(D, dim=0)
        losses['feature_loss'] = loss        
        return losses

    def text_loss(self, semantic_scores, semantic_labels):
        losses = {}
    ...
    def parse_losses(self, losses):
        ...
        log_vars['loss'] = loss
        log_vars['loss_wo_text'] = loss - log_vars['text_loss']
        ...
        if not self.semantic_only:
            if 'instance' in self.test_cfg.eval_tasks or 'panoptic' in self.test_cfg.eval_tasks:
                ...
                inst_feats, inst_map = self.clusters_voxelization(proposals_idx, proposals_offset,
                                                                  ...
                                                                  **self.instance_voxel_cfg)
                _, cls_scores, iou_scores, mask_scores, res_feats = self.forward_instance(inst_feats, inst_map)
                ret.update(res_feats=res_feats)
    ...
    def forward_instance(self, inst_feats, inst_map):
        ...
        # feats_ep = self.global_pool(feats, True)
        # print(feats_ep.features.shape)
    ...
    def global_pool(self, x, expand=False):
        ...
        x = x.replace_feature(torch.cat((x.features, x_pool_expand), dim=1))
        return x
